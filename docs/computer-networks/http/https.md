...
![](/https/symmetric-encryption.avif)

**对称密钥加密**一个很重要的特点就是使用相同的密钥进行加密和解密

![](/https/symmetric-encryption-problem.avif)
...
![](/https/asymmetric-encryption-creation.avif)

- A 使用从 B 收到的公钥加密数据，将密文发送给 B
- B 使用私钥解密从 A 接收到的密文，得到原始数据

![](/https/asymmetric-encryption-transport.avif)
...
![](/https/man-in-the-middle-attack.avif)

A 拿到的其实是 X 发送给他的伪造公钥，但是 A 无法察觉。

![](/https/man-in-the-middle-attack-2.avif)
...
![](/https/digital-certificate.avif)
