...
![](/http-cache/browser-cache-process-analysis.avif)
...
![](/http-cache/invalid-cache-control-and-request-resource.avif)

- **存在**该缓存结果和缓存标识，但该**结果已失效**，强缓存失效，则使用协商缓存（暂不分析）

![](/http-cache/invalid-cache-control-and-use-consult-cache.avif)

- 存在该缓存结果和缓存标识，且该结果尚未失效，强制缓存生效，直接返回该结果

![](/http-cache/invalid-cache-control.avif)
...
![](/http-cache/browser-cache-location.avif)
...
![](/http-cache/consult-cache-and-response-304.avif)

- 协商缓存**失效**，返回 200 和请求结果

![](/http-cache/consult-cache-and-response-200.avif)
